...
module COMP
  ...
  module EECI
    ...
    Hint_Text     = "Usa ←  e →  per scorrere le pagine"
    ...
    ComparisonTable = {        
      ...
      :param_rate     => [FEATURE_PARAM, 'Statistiche'],        # Parameter
      :special_flag   => [FEATURE_SPECIAL_FLAG, 'Speciale'],         # Special feature flag
      :element_rate   => [FEATURE_ELEMENT_RATE, 'Difesa Elementale'],    # Element Rate
      :debuff_rate    => [FEATURE_DEBUFF_RATE, 'Difesa Debuff'],      # Debuff Rate
      :state_rate     => [FEATURE_STATE_RATE, 'Difesa Stato'],        # State Rate
      :state_resist   => [FEATURE_STATE_RESIST, 'Immunità'],    # State Resist
      :atk_element    => [FEATURE_ATK_ELEMENT, 'Elemento Attacco'],      # Atk Element
      :atk_state      => [FEATURE_ATK_STATE, 'Stato Attacco'],          # Atk State
      :atk_speed      => [FEATURE_ATK_SPEED, 'Iniziativa'],          # Atk Speed
      :atk_times      => [FEATURE_ATK_TIMES, 'Numero Colpi'],         # Atk Times+
      :stype_add      => [FEATURE_STYPE_ADD, 'Specializzazione'],     # Add Skill Type
      :stype_seal     => [FEATURE_STYPE_SEAL, 'Proibizione Spec.'],# Disable Skill Type
      :skill_add      => [FEATURE_SKILL_ADD, 'Abilità/Proc/Passivi'],          # Add Skill
      :skill_seal     => [FEATURE_SKILL_SEAL, 'Proibizione Abilità'],     # Disable Skill
      :equip_wtype    => [FEATURE_EQUIP_WTYPE, 'Conoscenze Armi'],     # Equip Weapon
#     :equip_atype    => [FEATURE_EQUIP_ATYPE, 'Conoscenze Armature'],      # Equip Armor
      :equip_fix      => [FEATURE_EQUIP_FIX, 'Blocca Slot'],         # Lock Equip
      :equip_seal     => [FEATURE_EQUIP_SEAL, 'Sigilla Slot'],        # Seal Equip
      :action_plus    => [FEATURE_ACTION_PLUS, 'Azioni Aggiuntive'],    # Action Times+
      :party_ability  => [FEATURE_PARTY_ABILITY, 'Abilità Gruppo'],  # Party ability
    ...
    XParamName = {
      0   => "Precisione",  # HIT rate
      1   => "Evas. Fisica",  # EVAsion rate
      2   => "Critici",  # CRItical rate
      3   => "Evas. Critici",  # Critical EVasion rate
      4   => "Evas. Magica",  # Magic EVasion rate
      5   => "Rifl. Magica",  # Magic ReFlection rate
      6   => "Contrattacco",  # CouNTer attack rate
      7   => "Rigen. HP",  # Hp ReGeneration rate
      8   => "Rigen. MP",  # Mp ReGeneration rate
      9   => "Rigen. TP",  # Tp ReGeneration rate
    }
    #---------------------------------------------------------------------
    # * Name display for each sparam
    SParamName = {
      0   => "Minaccia",  # TarGet Rate
      1   => "Efficacia Guardia",  # GuaRD effect rate
      2   => "Efficacia Cure",  # RECovery effect rate
      3   => "Efficacia Pozioni",  # PHArmacology
      4   => "Tasso Costo MP",  # Mp Cost Rate
      5   => "Tasso Ricarica TP",  # Tp Charge Rate
      6   => "Debolezza Fisica",  # Physical Damage Rate
      7   => "Debolezza Magica",  # Magical Damage Rate
      8   => "Danni da Terreno",  # Floor Damage Rate
      9   => "Tasso di EXP",  # EXperience Rate
    }
    #---------------------------------------------------------------------
    # * Name display for party ability
    PartyAbilityName = {
      0   => "Dimezza Incontri",          # halve encounters
      1   => "No Incontri",          # disable encounters
      2   => "Difesa Agguato",         # disable surprise
      3   => "Att. a Sorpresa",        # increase preemptive strike rate
      4   => "Doppio Guadagno",             # double money earned
      5   => "Doppio Bottino",   # double item acquisition rate
    }
    #---------------------------------------------------------------------
